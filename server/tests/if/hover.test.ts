...
import { ifDocs, elsifDocs, hoverIfSyntax } from '../../src/if/hover';
...
describe('If Hover', () => {
    ...
    it('should return hover content when over elsif attribute', () => {
        const doc = TextDocument.create(
            'file://test.html',
            'html',
            1,
            '<div elsif="user.isAdmin"></div>'
        );
        ...
        const hover = hoverHandler({
            textDocument: { uri: doc.uri },
            position: { line: 0, character: 5 }, // inside `elsif`
        });

        expect(hover).not.toBeNull();
        expect((hover!.contents as any).value).toBe(elsifDocs);
    });
